...
// z               ! contour levels in increasing order
{
  ...
  for (j=(jub-1);j>=jlb;j--) {
    for (i=ilb;i<=iub-1;i++) {
      double temp1,temp2;
      temp1 = min(d[i*par.sizey+j],d[i*par.sizey+j+1]);
      temp2 = min(d[(i+1)*par.sizey+j],d[(i+1)*par.sizey+j+1]);
      dmin = min(temp1,temp2);
      temp1 = max(d[i*par.sizey+j],d[i*par.sizey+j+1]);
      temp2 = max(d[(i+1)*par.sizey+j],d[(i+1)*par.sizey+j+1]);
      dmax = max(temp1,temp2);
      if (dmax>=z[0]&&dmin<=z[nc-1]) {
	for (k=0;k<nc;k++) {
	  if (z[k]>=dmin&&z[k]<=dmax) {
		...
		// start from zero
		//=============================================================
		h[m] = d[(i+im[m-1])*par.sizey+j+jm[m-1]]-z[k];
		xh[m] = x[i+im[m-1]];
		yh[m] = y[j+jm[m-1]];
	      } else {
		h[0] = 0.25*(h[1]+h[2]+h[3]+h[4]);
