...
SECTION("MultiCellDS input/output")
    ...
    PARAMETER(bool, SF_all, false, \
    "Whether to compute the Safety Factor for all pixels")
    PARAMETER(bool, SF_one_pixel, false, \
    "Whether to compute the Safety Factor for single pixel")
    PARAMETER(int, SF_x, 0, \
...
SECTION("Cellular Potts Model - Initialisation")
    ...
    PARAMETER(int, celltype1_width, 3, "Initial width of cells of type 1")
    PARAMETER(int, celltype1_length, 20, "Initial length of cells of type 1")
    PARAMETER(int, celltype2_width, 3, "Initial width of cells of type 2")
    PARAMETER(int, celltype2_length, 4, "Initial length of cells of type 2")

    PARAMETER(std::string, micropatternmask, "None", \
    "Mask to indicate the geometry allowed by the cells")
    
    PARAMETER(bool, second_layer, false, \
            "Whether to use a strict division between two cell types")
