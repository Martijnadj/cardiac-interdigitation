...
void PDE::Secrete(CellularPotts *cpm) {
  ...
  for (int x=0;x<sizex;x++) {
    for (int y=0;y<sizey;y++) {
      // inside cells
      if (cpm->Sigma(x,y)) {
	      PDEvars[par.sizey*x+y]+=par.secr_rate[0]*dt;
      } else {
      // outside cells
	PDEvars[par.sizey*x+y]-=par.decay_rate[0]*dt*PDEvars[par.sizey*x+y];
      }
    }
  }
}
...
void PDE::InitializeAgeLayer(int l,double value,CellularPotts *cpm){
  for (int x=0;x<sizex;x++) {
    for (int y=0;y<sizey;y++) {
      if(PDEvars[l*par.sizex*sizey+par.sizey*x+y]>0) {
        PDEvars[l*par.sizex*sizey+par.sizey*x+y]=value;
      } else {
        PDEvars[l*par.sizex*sizey+par.sizey*x+y]=0.;
      }
    }
  }
}
